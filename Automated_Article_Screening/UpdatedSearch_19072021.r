...
all_new <- read_bibliography(file_names)
nrow(all_new) # no. of rows
# write to csv
write.csv(all_new, file = "all_aug2020_jul2021_articles.csv")
...
doi_duplicates_removed <- extract_unique_references(all_new, duplicates)
nrow(doi_duplicates_removed)
nrow(all_new)-nrow(doi_duplicates_removed) # how many duplicates were removed
...
nrow(all_new)-nrow(title_doi_duplicates_removed) # how many duplicates were removed overall
...
save.image("aug2020_jul2021_Dedup.Rdata")
...
write.csv(filtered_data, file = "aug2020_jul2021_V3_REGEX_for_manual_sorting.csv")

save.image("NEW_2021_V3_RGEX_ADHDscreen.Rdata")

# Export filtered dataframe to RIS format

write_bibliography(filtered_data, filename = "aug2020_jul2021_Phase1_filtered.ris", format = "ris")
